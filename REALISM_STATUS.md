...
- **真实回路升级为含已发表身份链**：98 节点/741 边；3 个左 hook 传入为**已发表身份感觉端口**（SNpp38），chief 9A 与 DNg100/74/12 共 22 个身份节点入图。六条件复跑（v1.0.1 后补齐路线图 §8 的"无连接组"对照）：full 8 去极化/胫节 1.003 rad；无本体感觉与**无连接组**均为 0 去极化回静息（此回路的驱动完全经由内部突触通路，感觉输入与结构连接缺一即静默）；rewired 0.658、shuffled_io 0.725——连接存在性必要，拓扑精确性决定终点。
