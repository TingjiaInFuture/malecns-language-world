...
> 2026-09-13 路线图实施：当前状态见 [REALISM_STATUS.md](REALISM_STATUS.md)。新增逐神经元编译器、受体电导参考核及验证；原有 3D 控制器现在明确归为工程沙盒，直接运行 server.py 必须加 `--engineering-sandbox`。下文为旧版运行说明，其中“生产”仅指历史工程模式，不能用于描述生理生产模型。旧前端摆腿/振翅动画已移除；没有关节状态时肢体保持静态。
